...
def round_corner(radius, outer, inner, fill):
    """Draw a round corner"""
    corner = Image.new('RGBA', (s+e, s+e), '#FFFFFF')
    idraw = ImageDraw.Draw(corner)
    create_pie(idraw,[s+e,s+e],s+e,fill=outer)                   
...
img = round_rectangle((we, he), s, "#EDEBE7", "#B6ABA0",'white')
#img.show()
#img.save('rounded_rect_out'+str(j)+'.png')
img = img.resize((w,h),Image.LANCZOS)
#img.show()
img.save('../figures/rounded_rect_outc'+str(j)+'.png')
